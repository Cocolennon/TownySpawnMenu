...
public class MetadataTowns implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(!sender.hasPermission("townyspawnmenu.set.town") || !sender.hasPermission("townyspawnmenu.set.admin")) { sender.sendMessage("§4You can't do that!"); return false; }
        if(!res.hasTown()) { sender.sendMessage("§4You aren't in a town!"); return false; }
        if(!res.isMayor()) { sender.sendMessage("§4You aren't  the mayor of your town!"); return false; }
        ...
        if(args.length > 1) {
            ...
            sender.sendMessage("§aThis town's item/block in the menu now is: " + material.name().toLowerCase());
        }else{
            Metadata.setBlockInMenu(res.getTownOrNull(), material.name());
            sender.sendMessage("§aYour town's item/block in the menu now is: " + material.name().toLowerCase());
        }
        return true;
    }
