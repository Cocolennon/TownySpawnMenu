...
public class MetadataNations implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(!sender.hasPermission("townyspawnmenu.set.nation") || !sender.hasPermission("townyspawnmenu.set.admin")) { sender.sendMessage("§4You can't do that!"); return false; }
        if(!res.hasNation()) { sender.sendMessage("§4You aren't in a nation!!"); return false; }
        if(!res.isKing()) { sender.sendMessage("§4You aren't the king of your nation!"); return false; }
        ...
        if(args.length > 1) {
            ...
            sender.sendMessage("§aThis nation's item/block in the menu now is: " + material.name().toLowerCase());
        }else{
            Metadata.setBlockInMenu(res.getNationOrNull(), material.name());
            sender.sendMessage("§aYour nation's item/block in the menu now is: " + material.name().toLowerCase());
        }
        return true;
    }
